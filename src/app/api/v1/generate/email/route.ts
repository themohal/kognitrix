...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { result, tokens, cost } = await generateEmail({
      prompt: body.prompt,
      type: body.type,
      tone: body.tone,
    });
    ...
    const { requestId, newBalance } = await deductCredits({
      ...
      requestPayload: { prompt: (body.prompt as string).slice(0, 500), type: body.type, tone: body.tone },
